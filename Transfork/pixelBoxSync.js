...
(function () {
    ...
    function scanAlpha(canvas, origin) {
        ...
        return { left: origin.left + left, top: origin.top + top, width: right - left + 1, height: bottom - top + 1 };
    }

    function transformValues(snapshot) {
        const text = snapshot?.style?.transform || "";
    ...
    function liveSnapshotRect() {
        const state = api.snapshotToolsPixel?.state;
        const snapshot = state?.snapshot;
        const base = state?.rect;
        const source = snapshotSource(snapshot);
        if (!state?.active || !snapshot || !base || !source) return null;
        ...
        const transform = transformValues(snapshot);
    ...
    function installPlaceOverride() {
        ...
        api.selectionBox.place = function (rect) {
            ...
            if (window.__transforkTransformActive && api.snapshotToolsPixel?.state?.active) {
                next = liveSnapshotRect() || rect;
            }
            else if (!busy) {
