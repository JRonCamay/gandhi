...
(function () {
    ...
    function vm() {
        return api.vm?.getVM?.() || window.vm || null;
    }
    ...
    function screenRect(bounds, stageCanvas, activeVM) {
        if (api.coords?.boundsToScreenRect) {
            return api.coords.boundsToScreenRect(bounds, stageCanvas, activeVM);
        }

        const nativeSize = activeVM.runtime.renderer.getNativeSize();
    ...
    function stageScale(stageCanvas, activeVM) {
        const rect = stageCanvas.getBoundingClientRect();
        const nativeSize = activeVM.runtime.renderer.getNativeSize();
        return nativeSize?.[0] ? rect.width / nativeSize[0] : 1;
    }
    ...
    function makeSnapshot(activeVM, target, drawable, stageCanvas, rect) {
        ...
        const scaleToScreen = stageScale(stageCanvas, activeVM);
    ...
    function apply(clientX, clientY) {
        ...
        state.snapshot.style.transform = "scale(" + sx + ", " + sy + ") rotate(" + rot + "deg) skew(" + skewX + "deg, " + skewY + "deg)";
        placeBox(box);
    }
    ...
    function bakeSkew(canvas, costumeData, shearX, shearY) {
        ...
        const cx = width / 2;
        const cy = height / 2;
        const points = [[0, 0], [width, 0], [width, height], [0, height]].map(point => ({
    ...
    function start(event, mode) {
        ...
        const overlayRect = overlay()?.getBoundingClientRect() || rect;
        ...
        Object.assign(state, {
            ...
            startAngle: Math.atan2(event.clientY - (overlayRect.top + overlayRect.height / 2), event.clientX - (overlayRect.left + overlayRect.width / 2))
        });
    ...
    api.registerModule260705_NS8Q2M("snapshotTools2", { state, bind, start, finish });
    bind();
})();
