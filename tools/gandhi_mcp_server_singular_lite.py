import json,os,re,tempfile,time,uuid,hashlib,shutil,subprocess
...
REPO_DIR=WORKSPACE_DIR/"gandhi"

mcp=FastMCP("Gandhi Lite",host="127.0.0.1",port=8001,streamable_http_path="/mcp",transport_security=TransportSecuritySettings(enable_dns_rebinding_protection=True,allowed_hosts=["127.0.0.1","localhost","perch-tripping-crushed.ngrok-free.dev"]))
...
ROOTS=[Path(x.strip()) for x in os.environ.get("LOCAL_FILE_ALLOWED_ROOTS",r"D:\Projects\Chad;D:\Projects\Chad\local\AI_MEMORY_FIN").split(";") if x.strip()]
class FPE(RuntimeError):pass
def now():return time.strftime("%Y-%m-%dT%H:%M:%S%z")
def ok(d=None):return {"status":"success",**(d or {})}
def er(c,e):return {"status":"error","err":c,"msg":str(e)}
def inn(p,r):
...
class FPow:
    def __init__(s,roots=None):
        s.r=[Path(x).expanduser().resolve() for x in (ROOTS if roots is None else roots)]
        if not s.r:raise FPE("no roots")
    def p(s,x,exist=False):
        if not str(x or "").strip():raise FPE("path req")
        p=Path(str(x)).expanduser().resolve()
        if not any(inn(p,r) or p==r for r in s.r):raise FPE("path blocked")
        if exist and not p.exists():raise FPE(f"missing:{p}")
        return p
    def info(s,path):
        p=s.p(path,True);z=p.stat();d={"path":str(p),"exists":True,"file":p.is_file(),"dir":p.is_dir(),"bytes":z.st_size,"mtime":z.st_mtime,"checked":now()}
        if p.is_file():
            t=p.read_text(encoding="utf-8",errors="replace")
            d|={"lines":len(t.splitlines()),"words":len(t.split()),"letters":sum(1 for c in t if c.isalpha()),"sha256":sha(p)}
        return ok(d)
    def wr(s,path,text,overwrite=False):
        p=s.p(path)
        if p.exists() and not overwrite:raise FPE("exists")
        b=str(text).encode("utf-8");wrb(p,b);return ok({"path":str(p),"bytes":len(b),"sha256":sha(p)})
    def ap(s,path,text):
        p=s.p(path);p.parent.mkdir(parents=True,exist_ok=True);b=str(text).encode("utf-8")
        with p.open("ab") as f:f.write(b);f.flush();os.fsync(f.fileno())
        return ok({"path":str(p),"appended":len(b),"bytes":p.stat().st_size,"sha256":sha(p)})
    def cp(s,src,dst,overwrite=False):
        a=s.p(src,True);b=s.p(dst)
        if not a.is_file():raise FPE("src not file")
        if b.exists() and not overwrite:raise FPE("exists")
        b.parent.mkdir(parents=True,exist_ok=True);shutil.copy2(a,b);return ok({"src":str(a),"dst":str(b),"bytes":b.stat().st_size,"sha256":sha(b)})
    def bw(s,files,overwrite=False):
        if not isinstance(files,list) or not files:raise FPE("files req")
        if len(files)>50:raise FPE("max 50")
        out=[];fail=[]
        for i,x in enumerate(files):
            try:
                if "source_path" in x:out.append(s.cp(x["source_path"],x.get("path",""),overwrite))
                else:out.append(s.wr(x.get("path",""),x.get("content",""),overwrite))
            except Exception as e:fail.append({"i":i,"err":str(e)})
        return ok({"count":len(out),"failed":fail,"files":out})
    def op(s,x):
        x=x or {};m=str(x.get("op") or x.get("operationMode") or "").lower()
        if m in {"info","stat"}:return s.info(x.get("path",""))
        if m in {"write","wr"}:return s.wr(x.get("path",""),x.get("content") or x.get("text",""),x.get("overwrite",False))
        if m in {"append","ap"}:return s.ap(x.get("path",""),x.get("content") or x.get("text",""))
        if m in {"copy","cp"}:return s.cp(x.get("source_path") or x.get("src",""),x.get("target_path") or x.get("dst",""),x.get("overwrite",False))
        if m in {"batch","batchwrite","bw"}:return s.bw(x.get("files",[]),x.get("overwrite",False))
        raise FPE("bad op")

def register_filepower_lite_tools(mcp,allowed_roots=None):
    s=FPow(allowed_roots)
    def call(c,f,*a,**k):
        try:return f(*a,**k)
        except FPE as e:return er(c,e)
        except Exception as e:return er(c,e)
    @mcp.tool()
    def filePower(fileObj:dict)->dict:
        """filePower(fileObj:{op,path,content,files,...})."""
        return call("FP01",s.op,fileObj)
    @mcp.tool()
    def wrFiles(fileObj:dict)->dict:
        """writeFiles(fileObj:{files,overwrite?}); fallback per file."""
        x=fileObj or {};return call("FP02",s.bw,x.get("files",[]),x.get("overwrite",False))
    return s
register_quietchat_lite_tools(mcp)
register_filepower_lite_tools(mcp)

@mcp.tool()
def ping()->dict:
    """health()."""
    return {"status":"success","server":"gandhi_lite"}
