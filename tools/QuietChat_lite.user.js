...
(function(){
  ...
  const ID="qc-lite-root",API="http://127.0.0.1:8765/quietchat/api",DAEMON="http://127.0.0.1:8766",POS="qc-lite-pos-v1",VER="0.3.43";
  ...
  function paintDomStatus(){
    ...
    document.querySelectorAll(`#${ID} .msg.a[data-mid="${CSS.escape(r.message_id)}"]`).forEach(el=>{el.textContent=txt;el.classList.toggle("live",!!domStatusSlot);el.classList.toggle("err",isErrStatus(txt));});
  }
  function isErrStatus(t){return /\b(error|failed|fail|limit|quota|rate|full|unavailable|try again)\b/i.test(String(t||""));}
  function visible(el){
  ...
  function wireFakeScroll(h){
    ...
    const bounceThumb=()=>{const r=bar.getBoundingClientRect(),hh=th.offsetHeight,span=Math.max(0,r.height-hh);th.style.transition="top .22s cubic-bezier(.2,1.4,.35,1)";th.style.top=Math.round(span/2)+"px";setTimeout(()=>{th.dataset.free="";th.style.transition="";updateBar();},260);};
    ...
    th.onpointerup=e=>{drag=false;stopEdge();try{th.releasePointerCapture(e.pointerId);}catch{};bounceThumb();};
    th.onpointercancel=()=>{drag=false;stopEdge();bounceThumb();};
  }
  function render(d,shift=""){
    ...
    for(const r of m){
      ...
      const live=r.message_id===domStatusMsgId&&domStatusSlot&&!r.assistant_reply,txt=r.assistant_reply||statusText(r);
      box.insertAdjacentHTML("beforeend",`<div class="msg a ${live?"live":""} ${live&&isErrStatus(txt)?"err":""}" data-mid="${esc(r.message_id||"")}">${hit?hi(txt,findText):esc(txt)}${fail}</div>`);
