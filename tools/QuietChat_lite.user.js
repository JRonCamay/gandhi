...
(function(){
  ...
  const ID="qc-lite-root",API="http://127.0.0.1:8765/quietchat/api",DAEMON="http://127.0.0.1:8766",POS="qc-lite-pos-v1",VER="0.3.41";
  ...
  function markSave(payload){
    const p=payload||{},id=p.message_id;
    if(!id)return;
    const r=last.find(x=>x.message_id===id);
    if(r)r.save_state=p.save_state||(p.ok===false?"error":"saved");
    document.querySelectorAll(`#${ID} .msg[data-mid="${CSS.escape(id)}"]`).forEach(el=>{
      el.querySelector(".save-x")?.remove();
      if(p.ok===false||p.save_state==="error"){
        const x=document.createElement("span");
        x.className="save-x";x.title=p.error||"Local save failed";x.textContent="×";
        el.appendChild(x);
      }
    });
  }
  ...
  function paintDomStatus(){
    ...
    document.querySelectorAll(`#${ID} .msg.a[data-mid="${CSS.escape(r.message_id)}"]`).forEach(el=>{el.textContent=txt;el.classList.toggle("live",!!domStatusSlot);});
  }
  function visible(el){
  ...
  function render(d,shift=""){
    ...
    for(const r of m){
      ...
      const fail=r.save_state==="error"?'<span class="save-x" title="Local save failed">×</span>':"";
      if(r.user_message){
        box.insertAdjacentHTML("beforeend",`<div class="msg u" data-mid="${esc(r.message_id||"")}">${hit?hi(r.user_message,findText):esc(r.user_message)}${fail}</div>`);
        box.insertAdjacentHTML("beforeend",pinRow(r,hit,"u"));
      }
      const live=r.message_id===domStatusMsgId&&domStatusSlot&&!r.assistant_reply;
      box.insertAdjacentHTML("beforeend",`<div class="msg a ${live?"live":""}" data-mid="${esc(r.message_id||"")}">${hit?hi(r.assistant_reply||statusText(r),findText):esc(r.assistant_reply||statusText(r))}${fail}</div>`);
  ...
  function wireEvents(){
    ...
    try{
      ...
      es.addEventListener("qc.blip",refresh);
      es.addEventListener("qc.data",refresh);
  ...
  function wireDoneObserver(){
    let sent="";
    const clean=t=>String(t||"").replace(/status:\s*\[done\]|\[done\]/ig,"").trim();
    const tick=()=>{
      const nodes=[...document.querySelectorAll('[data-message-author-role="assistant"],article')];
      const n=nodes.at(-1);if(!n)return;
      const t=(n.innerText||n.textContent||"").trim();
      if(!/status:\s*\[done\]|\[done\]\s*$/i.test(t))return;
      const body=clean(t),key=body.slice(-500);
      if(!body||key===sent)return;
      sent=key;
      req("/reply",{reply:body,status:"[DONE]"}).catch(()=>{});
    };
    new MutationObserver(()=>setTimeout(tick,250)).observe(document.body,{childList:true,subtree:true,characterData:true});
  }
  async function send(){
    if(busy)return;
