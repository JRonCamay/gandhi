...
(function(){
  ...
  function boot(){
    ...
    const h=document.createElement("div");h.id=ID;h.innerHTML=`
      <style>
        ...
        .newest{position:absolute;right:18px;bottom:74px;width:38px;height:38px;border-radius:999px!important;font-size:18px;z-index:2}
      </style>
      <section class=qcw><div class=qch><div><div class=qct>QuietChat Lite</div><div class=qcs>daemon view · 10 sliding</div></div><button class=scan>Scan</button><button class=hide>Dock</button></div><main class=qcb></main><button class=newest title="Newest">↓</button><div class=stat>ready</div><footer class=qcf><textarea placeholder="Message QuietChat..."></textarea><button class=send>Send</button></footer></section>`;
  ...
  function jumpNewest(){toNewest=true;load();}
