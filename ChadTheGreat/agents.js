...
(function () {
    ...
    function migrateAgents(agents) {
        ...
        for (const agent of agents) {
            ...
            if (!Object.prototype.hasOwnProperty.call(agent, "description")) {
                agent.description = "";
                changed = true;
            }
            if (
    ...
    function renderAgent(agent) {
        ...
        if (selected && !expanded) {
            box.appendChild(createEl("div", {
                ...
                style: { color: "#64748b", fontSize: "11px", padding: "5px 2px 0" }
            }));
        }
    ...
    function forcePaintAndGitGitButtons() {
        const panel = document.querySelector("#gandhi-chad-panel");
        if (!panel) return;

        const buttons = Array.from(panel.querySelectorAll("button"));
        const closeButton = buttons.find(btn => btn.textContent.trim() === "✕");
        const minimizeButton = buttons.find(btn => ["—", "-", "□"].includes(btn.textContent.trim()));
        ...
        let paintButton = panel.querySelector("#gandhi-chad-paint-button");

        if (!paintButton) {
            const source = minimizeButton || buttons.find(btn => btn.textContent.trim() === "🎨");
            if (source) {
                paintButton = source.cloneNode(true);
                source.replaceWith(paintButton);
            }
            else if (closeButton && closeButton.parentElement) {
                paintButton = window.Chad.ui.button("🎨", () => {}, { title: "Open Quick Sketch" });
                closeButton.parentElement.insertBefore(paintButton, closeButton);
            }
        }

        if (paintButton) {
            paintButton.id = "gandhi-chad-paint-button";
            paintButton.textContent = "🎨";
            paintButton.title = "Open Quick Sketch";
            paintButton.dataset.chadDrawPatched = "1";
        }

        if (paintButton && !panel.querySelector("#gandhi-chad-gitgit-button")) {
            const gitGitButton = paintButton.cloneNode(true);
            gitGitButton.id = "gandhi-chad-gitgit-button";
            gitGitButton.textContent = "🐒";
            gitGitButton.title = "GitGit (future)";
            gitGitButton.dataset.internalName = "GitGit";
            gitGitButton.addEventListener("click", event => {
                event.preventDefault();
                event.stopPropagation();
                alert("GitGit is reserved for the next module.");
            }, true);
            paintButton.insertAdjacentElement("afterend", gitGitButton);
        }
    }
